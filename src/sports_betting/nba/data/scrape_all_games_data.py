...
def clean_all_games_table(all_games_table: pd.DataFrame) -> pd.DataFrame:
    ...
    # Date to date object
    all_games_table["Date"] = all_games_table["Date"].apply(
        lambda x: dt.datetime.strptime(x, "%a, %b %d, %Y").date()
    )

    # Map teams to their standard three letter code
    all_games_table["Visitor/Neutral"] = all_games_table["Visitor/Neutral"].map(
        TEAM_CODE_DICT
    )
    all_games_table["Home/Neutral"] = all_games_table["Home/Neutral"].map(
        TEAM_CODE_DICT
    )

    # Drop useless columns, rename
    all_games_table.drop(columns=["Unnamed: 6", "Arena", "Notes"], inplace=True)
    all_games_table = all_games_table.rename(
        columns={
            "PTS": "VisitorPTS",
            "PTS.1": "HomePTS",
            "Unnamed: 7": "OT",
...
def insert_all_games_table(month: str, year: int) -> None:
    ...
    if all_games_table is None:
        print(
            "Failure: %(month)s, %(year)s all_games data does not exist"
            % {"month": month.capitalize(), "year": year}
        )
        conn.close()
        return
    ...
    with conn.cursor() as cursor:
        try:
            ...
            print(
                "Successfully executed %(month)s, %(year)s all_games data"
                % {"month": month.capitalize(), "year": year}
            )
        except Exception as e:
            print(e)
